...
pk = torch.load(HERE / a.path, map_location=dev, weights_only=False); pcfg = pk["config"]; NP = int(pcfg.get("np", 7))                                                  # places from the table
NODES = [str(d) for d in range(10)] + [str(10 ** k) for k in range(1, NP)]; nid = {n: i for i, n in enumerate(NODES)}; PLUS, TIMES = 0, 1; NR = 2
pmodel = ResonatE(len(NODES), 2 * NR, k=pcfg["k"], block=True, block_size=pcfg["block"]).to(dev); pmodel.load_state_dict(pk["model"]); pmodel.eval(); pmodel.requires_grad_(False)
...
with torch.no_grad():
    ...
    x = rng.integers(0, 10 ** NP, 6000); u = rng.integers(0, NP, 6000); d = rng.integers(1, 10 ** u + 1) * rng.choice([-1, 1], 6000); y = np.clip(x + d, 0, 10 ** NP - 1)
