...
ARGS = sys.argv[1:]; VALUED = ("--target", "--examples-shown", "--name"); FLAGS = ("--no-label",)
sys.argv = [sys.argv[0]] + [x for i, x in enumerate(ARGS) if x not in VALUED + FLAGS and (i == 0 or ARGS[i - 1] not in VALUED)]; NOLABEL = "--no-label" in ARGS
def opt(name, default=None): return ARGS[ARGS.index(name) + 1] if name in ARGS else default
TARGET = opt("--target", "sumd"); SHOWN = [int(v) for v in opt("--examples-shown", "123,90,5").split(",")]; NAME = opt("--name", {"sumd": "digitsum"}.get(TARGET, TARGET))   # the word the worked example uses: the program's label
...
_execute = g["execute"]; CYCLES = [NP + 1]                                                                            # loop cap: short during the search (3-digit inputs need no 15-cycle loops; runaway candidates would cost seconds each), full for verification
def execute(progs, xs, ys, depth=0, max_cycles=None, **kw): return _execute(progs, xs, ys, depth, max_cycles if max_cycles is not None else CYCLES[0], **kw)
g["execute"] = execute
...
TASKS = {"sumd": lambda x, y: digitsum(x), "double": lambda x, y: 2 * x, "triple": lambda x, y: 3 * x, "iseven": lambda x, y: EVEN if x % 2 == 0 else ODD,
         "square": lambda x, y: x * x, "sumsq": lambda x, y: x * x + y * y, "lhs": lambda x, y: x * x + y * y + 3, "check": lambda x, y: EQ if x * x + y * y + 3 == 7 else (LT if x * x + y * y + 3 < 7 else GT)}
KIND = "verdict" if TARGET in ("iseven", "check") else "number"
ARITY = 2 if TARGET in ("sumsq", "lhs", "check") else 1
XDIG = {"square": 3, "sumsq": 3, "lhs": 3, "check": 1}.get(TARGET, 3)
def events(x, y=0):
    ...
    if TARGET == "square": return [("call", "mul", x, x, x * x), ("answer", x * x)]                                  # 'square(7) = 7 mul 7 = 49'
    if TARGET == "sumsq": return [("call1", "square", x, x * x), ("call1", "square", y, y * y), ("call", "add", x * x, y * y, x * x + y * y), ("answer", x * x + y * y)]
    if TARGET == "lhs": s_ = x * x + y * y; return [("call", "sumsq", x, y, s_), ("value", 3), ("call", "add", s_, 3, s_ + 3), ("answer", s_ + 3)]
    if TARGET == "check":                                                                                           # 'check(2,0): 2x2 = 4, 0x0 = 0, 4 + 0 = 4, 4 + 3 = 7, 7 = 7 -> yes' — the expression inline, square the only named program
        sx, sy = x * x, y * y; v = sx + sy + 3; code = EQ if v == 7 else (LT if v < 7 else GT)
        return [("call1", "square", x, sx), ("call1", "square", y, sy), ("call", "add", sx, sy, sx + sy), ("value", 3), ("call", "add", sx + sy, 3, v), ("value", 7), ("call", "cmp", v, 7, code), ("answer", code)]
...
def shown_pairs(): return [(v, 0) for v in SHOWN] if ARITY == 1 else [(SHOWN[i], SHOWN[i + 1]) for i in range(0, len(SHOWN) - 1, 2)]
NAMED = sorted({e[1] for x, y in shown_pairs() for e in events(x, y) if e[0] in ("call", "call1")}); PLUMB = ("LOAD", "NUM", "COPY", "SET", "SHIFT", "ANSWER", "PARITY")
...
def realised(pl, ev, rows):
    ...
    elif kind == "call1": out = torch.tensor([any(c[0] == ev[1] and c[1] == ev[2] for c in pl.calls_seen[r]) for r in rows.tolist()], device=dev)      # a unary program: its first operand is what counts
    ...
    return out
def run_prefixes(prefixes, x, y=47):
    """execute each prefix (as an init-only program) on (x, y); returns the Places state"""
    progs = [{"kind": KIND, "init": list(pf), "body": [], "outro": [], "stop": 0} for pf in prefixes]
    return execute(progs, [x], [y], track=True)                                                                   # a non-zero second operand: no free zero in a slot
def slots_of(pl, r=0):
...
def derive(pl, ev, prefer):
    ...
    if ev[0] == "value":                                                                                            # a named constant: a digit in a free slot (a digit is a fine operand)
        return [[f"SET {s_}<{ev[1]}"] for s_ in free if 0 <= ev[1] <= 9] or [[]]
    ...
    if ev[0] == "call":
        ...
        cands = [[f"CALL {name} {u},{v}>{w}"] for u, v in pairs for w in outs(u, v)]
        if a_ == b_ and len(A) == 1 and free:                                                                        # the same value twice (7 x 7): copy it first, then call
            u = A[0]; cands += [[f"COPY {u}>{v}", f"CALL {name} {u},{v}>{w}"] for v in free for w in [u, v] + [f for f in free if f != v]]
        return cands
    if ev[0] == "parity": return [[f"PARITY {s_}>{u}"] for s_ in range(K) if st[s_] is not None and st[s_][0] == "dig" for u in free]
    if ev[0] == "call1":
        name, a_ = ev[1], ev[2]; A = holds(a_); others = [v for v in range(K) if st[v] is not None]
        return [[f"CALL {name} {u},{v}>{w}"] for u in A for v in others if v != u for w in [u] + free]
    if ev[0] == "answer": return [[f"ANSWER {s_}"] for s_ in holds(ev[1])]
    return []
def observe(x, prefer, beam=8, y=47):
    ...
    beams = [[]]; t0 = time.time()
    for ev in events(x, y):
        survivors = []
        with torch.no_grad():
            for tr in beams:
                pl0 = run_prefixes([tr], x, y); cands = derive(pl0, ev, prefer)
                if not cands: continue
                pl = run_prefixes([tr + c for c in cands], x, y); ok = realised(pl, ev, torch.arange(len(cands), device=dev))
...
TRUTH[TARGET] = ({"kind": KIND}, TASKS[TARGET]); REGRESSION[TARGET] = {"square": [(0, 3), (1, 0), (9, 9), (10, 5), (99, 0), (100, 1), (999, 2), (123, 4)], "sumsq": [(0, 0), (2, 0), (0, 2), (3, 4), (99, 99), (10, 100), (123, 456)], "lhs": [(0, 0), (2, 0), (0, 2), (1, 1), (99, 99), (10, 100)], "check": [(0, 0), (2, 0), (0, 2), (1, 1), (1, 2), (2, 2), (9, 9), (0, 1)], "iseven": [(0, 3), (1, 0), (10, 5), (11, 2), (999999999999, 1), (1000000000000, 4), (2468, 0), (13579, 7)], "sumd": [(0, 7), (9, 0), (99, 3), (999999999, 0), (1000000000, 5), (123456789, 0), (909090909090, 1), (999999999999, 0), (9999999999999, 2), (8888888888888, 0)], "double": [(0, 3), (5, 0), (99, 99), (999999, 1)], "triple": [(0, 9), (4, 0), (99, 1), (333333, 5)]}[TARGET]
_sample = sample
def sample(n, max_digits, task):
    if task != TARGET: return _sample(n, max_digits, task)
    md = min(max_digits, XDIG); return [rnum(int(rng.integers(1, md + 1))) for _ in range(n)], [rnum(int(rng.integers(1, md + 1))) for _ in range(n)]
g["sample"] = sample
t0 = time.time(); prefer = {}; traces = {}
for x, y in shown_pairs():
    print(f"\n== playing {TARGET}({x}{', ' + str(y) if ARITY == 2 else ''}) = {TASKS[TARGET](x, y)}", flush=True); tr = observe(x, prefer, y=y if ARITY == 2 else 47); traces[(x, y)] = tr
...
if demos:
    with torch.no_grad():
        pop = [from_demo() for _ in range(150)]; found_p = None; best, best_f = pop[0], -1; CYCLES[0] = XDIG + 4
        for gen in range(1, 61):
            digits_now = min(XDIG, 1 + (gen - 1) // 4)                                                              # small numbers first: 1-digit examples, one more digit every four generations
            xs, ys = sample(12, digits_now, TARGET); f, e = score(pop, TARGET, xs, ys); order = np.argsort(-f)
            if f[order[0]] > best_f: best, best_f = pop[order[0]], float(f[order[0]])
            print(f"  search gen {gen}/60  ({digits_now}-digit examples)  best {f[order[0]]:.3f} (exact {e[order[0]]:.3f})  mean {f.mean():.3f}  candidates {gen * 150}  [{time.time()-t0:.0f}s]", flush=True)
            if e[order[0]] >= 1.0:
                cands = [pop[i] for i in order if e[i] >= 1.0]; cands.sort(key=lambda p: len(p["init"]) + len(p["body"]) + len(p["outro"]))
                for cand in cands[:8]:
                    CYCLES[0] = NP + 1; xs2, ys2 = sample(48, XDIG + 2 if TARGET != "check" else 1, TARGET); _, ec = score([cand], TARGET, xs2, ys2); rx, ry = zip(*REGRESSION[TARGET]); _, er = score([cand], TARGET, list(rx), list(ry))
            ...
            pop = new
        CYCLES[0] = NP + 1; prog = found_p or best; v = verify(prog, TARGET, lengths=tuple(L for L in (1, 2, 3, 4, 6, 8, 12, 13) if L <= (XDIG + 3 if TARGET != "check" else 1)), n=60)
        ...
        if found_p is not None and not NOLABEL:                                                                      # accepted -> labelled in the library, automatically: from here on it is a call, not a search
            lib = json.load(open(HERE / "results/places/found_programs.json")); lib[NAME] = {**prog, "kind": KIND, "arity": ARITY, "named_by": "worked example", "shown": SHOWN}
...
(HERE / a.out).mkdir(parents=True, exist_ok=True); save_json(res, HERE / a.out / f"{TARGET}.json"); print(f"saved ({time.time()-t0:.0f}s)", flush=True)
